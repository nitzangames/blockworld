...
export function createMobileInput(root, { onAct }) {
  ...
  // The palette is full-width by default; on mobile, reserve the bottom-right corner so the
  // scrollable swatches end before the round action button instead of sliding under it.
  const palette = document.getElementById('palette');
  if (palette) palette.style.right = '120px';
  const move = { x: 0, y: 0 };
