...
//   worldName                     string
// }
export function createSession({ transport, getWorld, ownerId, myName, clientId, hooks }) {
  ...
  transport.onMessage((from, p) => {
    switch (p.t) {
      case T.WELCOME: // meta only; world arrives via SNAPSHOT
        ...
        if (hooks.onWelcome) hooks.onWelcome(joinedWorldName);
        break;
      case T.SNAPSHOT: {
      ...
      case T.EDIT:
        ...
        break;
      case T.POS:
        if (clientId && p.c === clientId) break; // ignore our own position if the relay echoes it
        hooks.onPos(from, p);
        break;
      case T.PERM:
  ...
  // Reused position payload — sent ~12x/sec; transport.send serializes synchronously, so reuse is safe.
  const _pos = { t: T.POS, x: 0, y: 0, z: 0, yaw: 0, pitch: 0, n: myName, c: clientId };
  ...
  return {
    ...
    sendPos(cam) {
      _pos.x = cam.pos[0]; _pos.y = cam.pos[1]; _pos.z = cam.pos[2];
      _pos.yaw = cam.yaw; _pos.pitch = cam.pitch;
      transport.send(_pos);
    },
